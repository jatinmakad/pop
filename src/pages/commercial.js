...
import { bedroomCheck, checkObj, checkObjNew, zoomCount } from "@/utils/constants";
  ...
  const getData = async (obj = filter, type, queryData = queryData) => {
    ...
    try {
      const { status, data } = await apiGet(
        apiPath.getCommercialSaleProperties,
        {
          page: obj?.page || 1,
  ...
  return (
    <>
      <div className="container inner_filter_form">
        <Head>
          <title>{t("MADA_PROPERTIES")} : Commercial</title>
        </Head>
        <div className="filter_form_wrap">
          <SearchForm setFilter={setFilter} tab={"commercial-buy"} />
        </div>
      </div>
      ...
      <div className="main_wrap">
        <div className="container">
          {queryData?.type !== "building" && propertyCount?.length > 0 && (
            <div className="inner_heading">
              <h2>
                {t("COMMERCIAL_PROPERTIES_FOR_SALE_IN")}
            ...
            <Container>
              <div className="inner_heading">
                <h2>
                  ...
                  <span className="text-green">{t("COMMERCIAL")}</span>
                </h2>
                <p>{t("FIND_HOMES_FIRST")}</p>
              </div>

              <SaleSlider type={"commercial-buy"} />
            </Container>
          </section>
        )}
        ...
        <section className="serach_result">
          <Container>
            <div className="inner_heading">
              <h2>
                {t("SEARCH_RESULT_FOR_COMMERCIAL")}{" "}
                    ...
                    <Button
                      onClick={() => {
                        if (!mapCondition) {
                          ...
                          router.push(
                            { pathname: `/map-search`, query: { ...router?.query, pageName: 'commercial-buy' } }
                          )
                        }
                      }}
