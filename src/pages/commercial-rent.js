...
function CommercialRent() {
  ...
  const { adsList, config } = useContext(AuthContext);
  ...
  const getData = async (obj = filter, type, queryData = queryData) => {
    ...
    try {
      const { status, data } = await apiGet(
        apiPath.getCommercialRentProperties,
        {
          page: obj?.page || 1,
          limit: obj?.limit || filter?.limit,
          priceType: queryData?.priceRangeType || '',
          propertyCategory:
  ...
  return (
    <>
      <div className="container inner_filter_form">
        <Head>
          <title>{t("MADA_PROPERTIES")} : Commercial Rent</title>
        </Head>
        <div className="filter_form_wrap">
          <SearchForm setFilter={setFilter} tab={"commercial-rent"} />
        </div>
      </div>
      ...
      <div className="main_wrap">
            ...
            <Container>
              <div className="inner_heading">
                <h2>
                  ...
                  <span className="text-green">{"Commercial Rent"}</span>
                </h2>
                <p>{t("FIND_HOMES_FIRST")}</p>
              </div>

              <SaleSlider pageType={"rent"} type={"commercial-rent"} />
            </Container>
          </section>
        )}
        ...
        <section className="serach_result">
          <Container>
            <div className="inner_heading">
              <h2>
                {t("SEARCH_RESULT_FOR_COMMERCIAL_RENT")}{" "}
                <span className="text-green">{config?.country}</span>
              </h2>
              <p className="fw-normal">
                {properties?.totalDocs} {t("RESULTS")}
              </p>
            </div>
            <Row>
              <Col xl={9} className={!isEmpty(adsList) && adsList?.display?.includes("commercial") ? '' : "without_sidebar"}>
                <div
                    ...
                    <Button
                      onClick={() => {
                        if (!mapCondition) {
                          ...
                          router.push(
                            { pathname: `/map-search`, query: { ...router?.query, pageName: 'commercial-rent', priceType: 'yearly' } }
                          )
                        }
                      }}
              ...
              {!isEmpty(adsList) && adsList?.display?.includes("commercial") && (
