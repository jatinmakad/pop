...
function Rent() {
  ...
  const getData = async (obj = filter, type, queryData = queryData) => {
    ...
    try {
      const { status, data } = await apiGet(apiPath.getRentProperties, {
        page: obj?.page || 1,
  ...
  return (
    <>
      <div className="container inner_filter_form">
        <Head>
          <title>
            {t("MADA_PROPERTIES")} : {t("RENT")}
          </title>
        </Head>
        <div className="filter_form_wrap">
          <SearchForm setFilter={setFilter} type={"rent"} tab={"rent"} />
        </div>
      </div>
      ...
      <div className="main_wrap">
        <div className="container">
          {queryData?.type !== "building" && propertyCount?.length > 0 && (
            <div className="inner_heading">
              <h2>
                {t("RESIDENTIAL_PROPERTIES_FOR_RENT")}{" "}
        ...
        {isEmpty(queryData) && (
          <section className="properties_for_rent py-4 py-md-5">
            <Container>
              ...
              <SaleSlider pageType={"rent"} type={"rent"} />
            </Container>
          </section>
        )}
        ...
        <section className="serach_result">
          <Container>
            <div className="inner_heading">
              <h2>
                {t("SEARCH_RESULT_FOR_RESIDENTIAL_PROPERTIES")}{" "}
            ...
            <Row>
              <Col xl={9} className={!isEmpty(adsList) && adsList?.display?.includes("rent") ? '' : "without_sidebar"}>
                <div
                    ...
                    <Button
                      onClick={() => {
                        if (!mapCondition) {
                          ...
                          router.push(
                            { pathname: `/map-search`, query: { ...router?.query, pageName: "rent", priceType: 'yearly' } }
                          )
                        }
                      }}
              ...
              {!isEmpty(adsList) && adsList?.display?.includes("rent") && (
